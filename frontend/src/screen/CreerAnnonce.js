...
const CreerAnnonce = () => {
  ...
  const handleImageUpload = async(e) => {
    ...
    // Envoi des images au serveur PHP
    const formDataForServer = new FormData();
    formDataForServer.append('annonce_id', 'docRef.id');  // Remplacez par l'ID de l'annonce
    newImages.forEach(image => {
      formDataForServer.append('images[]', image.file);
    });

    try {
      const response = await fetch('http://localhost/ECOPARTAGE/Backend/upload_images.php', {
        method: 'POST',
      ...
      const textResponse = await response.text();  // Récupère la réponse sous forme de texte brut
